/*
*  
*  mandelbrot.c
*  This is a modified version of the simple server code designed to
*  serve madlebrot tiles over HTTP.
*
*  Modifications by Harry J.E Day and Jade Giacoppo
*  April 2014
*
*  Based on the code by:
*  Richard Buckland 28/01/11, 30/3/14.
*  Licensed under Creative Commons SA-BY-NC 3.0, share freely.
*
*/
 
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <assert.h>
#include <netinet/in.h>
#include <unistd.h>


#include "mandelbrot.h"
#include "pixelColor.h"


#define ESCAPE_DISTANCE 4
#define MAX_ITERATIONS 256

#define SIZE 512
#define SIMPLE_SERVER_VERSION 2.0
#define REQUEST_BUFFER_SIZE 1000
#define DEFAULT_PORT 7192
#define NUMBER_OF_PAGES_TO_SERVE 1000000
#define GET_STRING "GET "
#define Z_CHAR 'z'
#define X_CHAR 'x'
#define Y_CHAR 'y'
#define DOUBLE_MAX_DIGITS 1079
#define NOT_BMP 1
#define BMP 0
#define FILE_PREFIX ".bmp"
#define BMP_HEADER 0x42, 0x4D, 0x38, 0x0, 0x0C, 0, 0,0,0,0, 0x36, 0x0, 0x0, 0x0, 0x28, 0x0, 0x0, 0x0, 0x0, 0x02, 0x00, 0x0, 0x0, 0x2, 0x0, 0x0, 0x1,0x0,0x18,0x0,0x0,0x0, 0x0,0x0, 0x02, 0x0, 0x0C, 0x0, 0x13, 0x0B, 0x00, 0x0, 0x13, 0x0B, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0
#define BYTES_PER_PIXEL 3
#define BMP_HEADER_SIZE 54

 
double calculateSquareSize (int zoom);
double cornerValue(double center, double squareSize);
void printMandelbrot(double x, double y, double squareSize);
double squareY (double x, double y);
double squareX (double x, double y);
void tests();
void writeHeader (FILE *file);
void printServer();
int waitForConnection (int serverSocket);
int makeServerSocket (int portno);
void serveHTML (int socket);
int processReqeust(char request[], double *x, double *y,double *z);
void serveBMP(double xIn, double yIn, double zIn, int socket);
void httpHeader (int socket);
void servePage( int serverSocket);
void closeServer(int serverSocket); 
 
 
int main (int argc, char* argv[]) {
    tests();
    
    printServer();
    int serverSocket = makeServerSocket(DEFAULT_PORT);
    
    
    servePage(serverSocket);

    // close the server connection after we are done- keep aust beautiful
    closeServer(serverSocket);
  
    return EXIT_SUCCESS;
}


double calculateSquareSize (int zoom) {
   //square size = 2^(-zoom)
   int count = 1;
     //Starts count at 2^1, rather than 2^0, which is one
   double squareSize = (double)2.0;
   while (count < zoom){
      squareSize *= (double)2.0;
      count++;
   }
   squareSize = ((double)1.0)/squareSize;
   return squareSize;
}   


double cornerValue(double center, double squareSize){
   //long long numberOfSquares = 256;//(((double)SIZE)/squareSize)*(((double)SIZE)/squareSize);
   //double corner = center - 256*squareSize;//(numberOfSquares/2*squareSize);
   
    // times square size to convert SIZE/2 from pixels to the values
    // of the x/y axis
   double corner = center - (SIZE/2)*squareSize;
    //As the mandelbrot vale is calculated from the value of the center of the tile
   double cornerCenter = corner + squareSize/2;
    //cornerCenter is the x/y coordinate of the center of the left bottom tile
   return cornerCenter;
}

//calculates the number of steps/iterations before a point 'escapes' the mandlebrot set
int escapeSteps(double x, double y){
   double zReal = 0;
   double zImaginary = 0;
   int steps = 0;
   double distanceSquared = 0; 
   double temp;
   
   while (distanceSquared < ESCAPE_DISTANCE && steps < MAX_ITERATIONS){
      temp = zReal;
      zReal = squareX(zReal, zImaginary) + x;
      zImaginary = squareY(temp, zImaginary) + y;
      distanceSquared = zReal*zReal + zImaginary*zImaginary;
      steps ++;
   }
   return steps;
}

double squareX (double x, double y){
   x = x*x - y*y;
   return x;
}

double squareY (double x, double y){
   y = ((double)2.0)*x*y;
     //to square complex number: a^2-b^2 + 2abi 
     //this is the complex 2abi component
   return y;
}

/*
 * This function prints out the server information
 */
void printServer(){
    printf ("************************************\n");
    printf ("Starting simple server %f\n", SIMPLE_SERVER_VERSION);
    printf ("Serving almond bread since 1751\n");
    printf ("Access this server at http://localhost:%d/\n", DEFAULT_PORT);
    printf ("************************************\n");
} 
 

/*
 * This method analyses the request to determine what we should sever
 * returns TRUE if a bmp file has been requested
 * otherwise returns false
 */
int processReqeust(char request[], double *x, double *y,double *z) {
    int bmp = NOT_BMP;
    //loop through each line of the request
    
    
    char *line = strtok(request, "\n");
    while ((line != NULL) && (bmp != BMP)) { //loop until last line
        //we want the GET line
        if(strncmp(line, GET_STRING, 3) == 0) {
            
            //locate the position of x, y and z in request
            char *xIndex = strchr(line, X_CHAR);
            char *yIndex = strchr(line, Y_CHAR);
            char *zIndex = strchr(line, Z_CHAR);
            
            //get rid of starting 'x', 'y' or 'z'
            xIndex++;
            yIndex++;
            zIndex++;
            
            //TODO: add code to handle invalid input
            assert(xIndex != NULL);
            assert(yIndex != NULL);
            assert(zIndex != NULL);
            
            //we need this to work out lengths
            char *dotIndex = strstr(line, FILE_PREFIX);
            
            if(dotIndex == NULL) {
                bmp = NOT_BMP;
            } else {
            
                //check its a bmp request
                if(strpbrk(dotIndex, FILE_PREFIX) == NULL) {
                    bmp = NOT_BMP;
                } else {
                    //note: extra character for terminator
                    const int xSize = yIndex - xIndex;
                    const int ySize = zIndex - yIndex;
                    const int zSize = dotIndex - zIndex +1 ;
                    
                    char xCord[xSize];
                    char yCord[ySize];
                    char zCord[zSize];
                    
                    //get the individual strings
                    char* xStart = strncpy(xCord,xIndex,xSize-2);
                    xCord[xSize-1] = 0;
                    
                    char* yStart = strncpy(yCord,yIndex,ySize-2);
                    yCord[ySize-1] = 0; 
                    
                    char* zStart = strncpy(zCord,zIndex,zSize-1);
                    zCord[zSize-1] = 0;
                   
                    //convert to floats
                    (*x) = atof(xStart);
                    (*y) = atof(yStart);
                    (*z) = atof(zStart);
                    
                    bmp = BMP;
                } 
            }
            //TODO: we don't need to keep looping
            
        }
        
        line = strtok(NULL, "\n");
    }
    
    return bmp;
}
 
void serveHTML(int socket) {
    const char* message =
        "HTTP/1.1 417 OK\r\n"
        "Content-Type: text/html\r\n"
        "\r\n"
        "<!DOCTYPE html>\n"
        "<html>"
        "   <body>"
        "   <script src=\"http://almondbread.cse.unsw.edu.au/tiles.js\"></script>"
        "   </body>"
        "</html>";

    // echo the http response to the console for debugging purposes
    printf ("VVVV about to send this via http VVVV\n");
    printf ("%s\n", message);
    printf ("^^^^ end of message ^^^^\n");

    // send the http response to the web browser which requested it
    send (socket, message, strlen(message), 0);
}


void serveBMP(double xIn, double yIn, double zIn, int socket) {
    
    httpHeader(socket);
    
    //BMP Header
    unsigned char bmp[BMP_HEADER_SIZE + SIZE*SIZE*BYTES_PER_PIXEL];
    unsigned char bmpHeader[] = {BMP_HEADER};
    
    //put header in bmp
    int offset = 0;
    while(offset < BMP_HEADER_SIZE) {
        bmp[offset] = bmpHeader[offset];
        offset++;
    }
   
    double squareSize = calculateSquareSize(zIn);
      //first x and y coordinate value
    double mandelbrotX = cornerValue(xIn,squareSize);
    double mandelbrotY = cornerValue(yIn,squareSize);
    
    double initialMandelbrotX = mandelbrotX;
    
    int column =  0;
    int row = 0;
    while(row < SIZE) {
        while(column < SIZE) {
            int steps = escapeSteps(mandelbrotX,mandelbrotY);
             //G,B,R
            bmp[offset] = stepsToRed (steps);
            bmp[offset+1] = stepsToBlue (steps);
            bmp[offset+2] =  stepsToGreen (steps);
            
            offset += BYTES_PER_PIXEL;
            
            column++;
            mandelbrotX += squareSize;
        }
        column = 0;
        mandelbrotX = initialMandelbrotX;
        row++;
        mandelbrotY += squareSize;
    }
    int warning = write(socket, bmp, sizeof(bmp));
    printf("write returned %d", warning);
}


//writes HTTP header
void httpHeader (int socket){
    //HTTP Header
    const char* message = "HTTP/1.0 200 OK\r\n"
        "Content-Type: image/bmp\r\n"
        "\r\n";
    int warning = write(socket, message, strlen(message));
    printf("Write returned %d", warning);
} 
 
 
// start the server listening on the specified port number
int makeServerSocket (int portNumber) {
 
    // create socket
    int serverSocket = socket (AF_INET, SOCK_STREAM, 0);
    assert (serverSocket >= 0);
    // check there was no error in opening the socket
 
    // bind the socket to the listening port  (7191 in this case)
    struct sockaddr_in serverAddress;
    serverAddress.sin_family      = AF_INET;
    serverAddress.sin_addr.s_addr = INADDR_ANY;
    serverAddress.sin_port        = htons (portNumber);
 
    // tell the server to restart immediately after a previous shutdown
    // even if it looks like the socket is still in use
    // otherwise we might have to wait a little while before rerunning the
    // server once it has stopped
    const int optionValue = 1;
    setsockopt (serverSocket, SOL_SOCKET, SO_REUSEADDR, &optionValue, sizeof (int));
 
    int bindSuccess = bind (serverSocket, (struct sockaddr*)&serverAddress, sizeof (serverAddress));
 
    assert (bindSuccess >= 0);
    // if this assert fails wait a short while to let the operating
    // system clear the port before trying again
 
    return serverSocket;
}
 
// wait for a browser to request a connection,
// returns the socket on which the conversation will take place
int waitForConnection (int serverSocket) {
    // listen for a connection
    const int serverMaxBacklog = 10;
    listen (serverSocket, serverMaxBacklog);
 
    // accept the connection
    struct sockaddr_in clientAddress;
    socklen_t clientLen = sizeof (clientAddress);
    int connectionSocket = accept (serverSocket, (struct sockaddr*)&clientAddress, &clientLen);
    assert (connectionSocket >= 0);
    // check for connection error
 
    return connectionSocket;
}

void servePage( int serverSocket){
   char request[REQUEST_BUFFER_SIZE];
   int numberServed = 0;
   while (numberServed < NUMBER_OF_PAGES_TO_SERVE) {
        
        double x, y, z; //mandlebrot cordinates;
        
        printf ("*** So far served %d pages ***\n", numberServed);
 
        // STEP 1. wait for a request to be sent from a web browser, 
        // then open a new connection for this conversation
        int connectionSocket = waitForConnection(serverSocket);
 
        // STEP 2. read the first line of the request
        int bytesRead = recv (connectionSocket, request, sizeof(request) - 1, 0);
        assert (bytesRead >= 0);
        // check that we were able to read some data from the connection
 
        // echo entire request to the console for debugging
        printf (" *** Received http request ***\n %s\n", request);
        
        
        int bmp = processReqeust(request, &x, &y, &z);

 

        // STEP 3. decide between html or bmp
        printf (" *** Sending http response ***\n");
        
        if(bmp == NOT_BMP) {
            serveHTML (connectionSocket);
        } else {
            serveBMP(x,y,z,connectionSocket);
        }
        // STEP 4. close the connection after sending the page- keep aust beautiful
        close (connectionSocket);
        numberServed++;
    }
}

void closeServer(int serverSocket){
    printf ("** shutting down the server **\n");
    close (serverSocket);
}

void tests(){
   assert (escapeSteps (100.0, 100.0) == 1);
   assert (escapeSteps (0.0, 0.0) == 256);
    
   assert (escapeSteps (-1.5000000000000, -1.5000000000000) == 1);
   assert (escapeSteps (-1.4250000000000, -1.4250000000000) == 1);
   assert (escapeSteps (-1.3500000000000, -1.3500000000000) == 2);
   assert (escapeSteps (-1.2750000000000, -1.2750000000000) == 2);
   assert (escapeSteps (-1.2000000000000, -1.2000000000000) == 2);
   assert (escapeSteps (-1.1250000000000, -1.1250000000000) == 3);
   assert (escapeSteps (-1.0500000000000, -1.0500000000000) == 3);
   assert (escapeSteps (-0.9750000000000, -0.9750000000000) == 3);
   assert (escapeSteps (-0.9000000000000, -0.9000000000000) == 3);
   assert (escapeSteps (-0.8250000000000, -0.8250000000000) == 4);
   assert (escapeSteps (-0.7500000000000, -0.7500000000000) == 4);
   assert (escapeSteps (-0.6750000000000, -0.6750000000000) == 6);
   assert (escapeSteps (-0.6000000000000, -0.6000000000000) == 12);
   assert (escapeSteps (-0.5250000000000, -0.5250000000000) == 157);
   assert (escapeSteps (-0.4500000000000, -0.4500000000000) == 256);
   assert (escapeSteps (-0.3750000000000, -0.3750000000000) == 256);
   assert (escapeSteps (-0.3000000000000, -0.3000000000000) == 256);
   assert (escapeSteps (-0.2250000000000, -0.2250000000000) == 256);
   assert (escapeSteps (-0.1500000000000, -0.1500000000000) == 256);
   assert (escapeSteps (-0.0750000000000, -0.0750000000000) == 256);
   assert (escapeSteps (-0.0000000000000, -0.0000000000000) == 256);
    
   assert (escapeSteps (-0.5400000000000, 0.5600000000000) == 256);
   assert (escapeSteps (-0.5475000000000, 0.5650000000000) == 58);
   assert (escapeSteps (-0.5550000000000, 0.5700000000000) == 28);
   assert (escapeSteps (-0.5625000000000, 0.5750000000000) == 22);
   assert (escapeSteps (-0.5700000000000, 0.5800000000000) == 20);
   assert (escapeSteps (-0.5775000000000, 0.5850000000000) == 15);
   assert (escapeSteps (-0.5850000000000, 0.5900000000000) == 13);
   assert (escapeSteps (-0.5925000000000, 0.5950000000000) == 12);
   assert (escapeSteps (-0.6000000000000, 0.6000000000000) == 12);
    
   assert (escapeSteps (0.2283000000000, -0.5566000000000) == 20);
   assert (escapeSteps (0.2272500000000, -0.5545000000000) == 19);
   assert (escapeSteps (0.2262000000000, -0.5524000000000) == 19);
   assert (escapeSteps (0.2251500000000, -0.5503000000000) == 20);
   assert (escapeSteps (0.2241000000000, -0.5482000000000) == 20);
   assert (escapeSteps (0.2230500000000, -0.5461000000000) == 21);
   assert (escapeSteps (0.2220000000000, -0.5440000000000) == 22);
   assert (escapeSteps (0.2209500000000, -0.5419000000000) == 23);
   assert (escapeSteps (0.2199000000000, -0.5398000000000) == 26);
   assert (escapeSteps (0.2188500000000, -0.5377000000000) == 256);
   assert (escapeSteps (0.2178000000000, -0.5356000000000) == 91);
   assert (escapeSteps (0.2167500000000, -0.5335000000000) == 256);
    
   assert (escapeSteps (-0.5441250000000, 0.5627500000000) == 119);
   assert (escapeSteps (-0.5445000000000, 0.5630000000000) == 88);
   assert (escapeSteps (-0.5448750000000, 0.5632500000000) == 83);
   assert (escapeSteps (-0.5452500000000, 0.5635000000000) == 86);
   assert (escapeSteps (-0.5456250000000, 0.5637500000000) == 74);
   assert (escapeSteps (-0.5460000000000, 0.5640000000000) == 73);
   assert (escapeSteps (-0.5463750000000, 0.5642500000000) == 125);
   assert (escapeSteps (-0.5467500000000, 0.5645000000000) == 75);
   assert (escapeSteps (-0.5471250000000, 0.5647500000000) == 60);
   assert (escapeSteps (-0.5475000000000, 0.5650000000000) == 58);
    
   assert (escapeSteps (0.2525812510000, 0.0000004051626) == 60);
   assert (escapeSteps (0.2524546884500, 0.0000004049095) == 61);
   assert (escapeSteps (0.2523281259000, 0.0000004046564) == 63);
   assert (escapeSteps (0.2522015633500, 0.0000004044033) == 65);
   assert (escapeSteps (0.2520750008000, 0.0000004041502) == 67);
   assert (escapeSteps (0.2519484382500, 0.0000004038971) == 69);
   assert (escapeSteps (0.2518218757000, 0.0000004036441) == 72);
   assert (escapeSteps (0.2516953131500, 0.0000004033910) == 74);
   assert (escapeSteps (0.2515687506000, 0.0000004031379) == 77);
   assert (escapeSteps (0.2514421880500, 0.0000004028848) == 81);
   assert (escapeSteps (0.2513156255000, 0.0000004026317) == 85);
   assert (escapeSteps (0.2511890629500, 0.0000004023786) == 89);
   assert (escapeSteps (0.2510625004000, 0.0000004021255) == 94);
   assert (escapeSteps (0.2509359378500, 0.0000004018724) == 101);
   assert (escapeSteps (0.2508093753000, 0.0000004016193) == 108);
   assert (escapeSteps (0.2506828127500, 0.0000004013662) == 118);
   assert (escapeSteps (0.2505562502000, 0.0000004011132) == 131);
   assert (escapeSteps (0.2504296876500, 0.0000004008601) == 150);
   assert (escapeSteps (0.2503031251000, 0.0000004006070) == 179);
   assert (escapeSteps (0.2501765625500, 0.0000004003539) == 235);
   assert (escapeSteps (0.2500500000000, 0.0000004001008) == 256);
    
   assert (escapeSteps (0.3565670191423, 0.1094322101123) == 254);
   assert (escapeSteps (0.3565670191416, 0.1094322101120) == 255);
   assert (escapeSteps (0.3565670191409, 0.1094322101118) == 256);
   assert (escapeSteps (0.3565670950000, 0.1094322330000) == 222);
   assert (escapeSteps (0.3565670912300, 0.1094322318625) == 222);
   assert (escapeSteps (0.3565670874600, 0.1094322307250) == 222);
   assert (escapeSteps (0.3565670836900, 0.1094322295875) == 222);
   assert (escapeSteps (0.3565670799200, 0.1094322284500) == 222);
   assert (escapeSteps (0.3565670761500, 0.1094322273125) == 222);
   assert (escapeSteps (0.3565670723800, 0.1094322261750) == 222);
   assert (escapeSteps (0.3565670686100, 0.1094322250375) == 223);
   assert (escapeSteps (0.3565670648400, 0.1094322239000) == 223);
   assert (escapeSteps (0.3565670610700, 0.1094322227625) == 224);
   assert (escapeSteps (0.3565670573000, 0.1094322216250) == 225);
   assert (escapeSteps (0.3565670535300, 0.1094322204875) == 256);
   assert (escapeSteps (0.3565670497600, 0.1094322193500) == 256);
   assert (escapeSteps (0.3565670459900, 0.1094322182125) == 237);
   assert (escapeSteps (0.3565670422200, 0.1094322170750) == 233);
   assert (escapeSteps (0.3565670384500, 0.1094322159375) == 232);
   assert (escapeSteps (0.3565670346800, 0.1094322148000) == 232);
   assert (escapeSteps (0.3565670309100, 0.1094322136625) == 232);
   assert (escapeSteps (0.3565670271400, 0.1094322125250) == 233);
   assert (escapeSteps (0.3565670233700, 0.1094322113875) == 234);
   assert (escapeSteps (0.3565670196000, 0.1094322102500) == 243);
    
    
   printf (" ... escapeSteps tests passed!\n");

}






